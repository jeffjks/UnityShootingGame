...
namespace ND_VariaBULLET
{
    public class AutomateLinear : AutomateBase
    {
        ...
        [Tooltip("Sets the duration(frame) at which values gradually modify between From and To.")]
        public int Duration;
        ...
        void Update()
        {
            ...
            accumulator += 1;
            accumulatorTotal += 1;
            controlLink[Destination]((method(From, To, Duration)));
        }

        protected override float SinglePass(float from, float to, int duration)
        {
            //float relativeSpeed = getRelativeSpeed(from, to, speed);
            float current = lerpMethod[Convert.ToInt16(Ease)](from, to, Curve.Evaluate((float) accumulator / duration));
        ...
        protected override float Continuous(float from, float to, int duration)
        {
            if (TotalCycles == 0)
            {
                float direction = (from <= to) ? 1 : -1;
                return from + (float) accumulator / duration * (to - from) * direction;
            }
        ...
        protected override float PingPong(float from, float to, int duration)
        {
            //float relativeSpeed = getRelativeSpeed(from, to, speed);
            float curve = Curve.Evaluate(Mathf.PingPong((float) accumulator / duration, 1));
        ...
        protected override float Randomized(float from, float to, int duration)
        {
            if (duration == accumulator)
            {
        ...
        /*
        private float getRelativeSpeed(float from, float to, int duration)
        {
