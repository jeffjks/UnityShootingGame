...
namespace ND_VariaBULLET
{
    public class ShotHoming : ShotBaseColorizable, IRePoolable
    {
        ...
        private void linear()
        {
            transform.position += new Vector3(scaledSpeed * Trajectory.x / Application.targetFrameRate * Time.timeScale, scaledSpeed * Trajectory.y / Application.targetFrameRate * Time.timeScale, 0);
        }

        private void moveTowards()
        {
            transform.position = Vector3.MoveTowards(transform.position, lastKnownPos, scaledSpeed / Application.targetFrameRate * Time.timeScale);
        }
