...
namespace ND_VariaBULLET
{
    public class ShotLaser : ShotLaserBase
    {
        ...
        {
            float laserExtension = Mathf.MoveTowards(mainGo.transform.localScale.x, maxDistance, speed / Application.targetFrameRate * Time.timeScale);
        ...
        private void laserCollision(RaycastHit2D ray, float speed)
        {
            ...
            {                                                                           //Fix for left facing
                float laserExtension = Mathf.MoveTowards(mainGo.transform.localScale.x, (distance * globalDirection * ppu) - (blastArea * globalDirection), speed / Application.targetFrameRate * Time.timeScale);
                mainGo.transform.localScale = new Vector3(laserExtension, mainGo.transform.localScale.y, 1);
