...
namespace ND_VariaBULLET
{
    public class ShotLaserMask : ShotLaserBase
    {
        ...
        public override void Start()
        {
            ...
            originGo = initLaserEnds(new GameObject("Origin"), new Vector2(0, 0), OriginImg, rend.sortingOrder + 1, false);
            mainGo = initLaserMain(new GameObject("Main"), new Vector2(MainAdjust * globalDirection, 0), MainImg, rend.sortingOrder);
            tipGo = initLaserEnds(new GameObject("Tip"), new Vector2((MainAdjust + TipAdjust) * globalDirection, 0), TipImg, rend.sortingOrder + 2, true);
        ...
        {
            float laserExtension = Mathf.MoveTowards(mainMask.transform.localScale.x, maxDistance * globalDirection, speed / Application.targetFrameRate * Time.timeScale);
        ...
        private void laserCollision(RaycastHit2D ray, float speed)
        {
            ...
            {
                float laserExtension = Mathf.MoveTowards(mainMask.transform.localScale.x, (distance * ppu) - blastArea, speed / Application.targetFrameRate * Time.timeScale);
                mainMask.transform.localScale = new Vector3(laserExtension, mainMask.transform.localScale.y, 1);
        ...
        private GameObject initLaserEnds(GameObject part, Vector3 initPosition, Sprite[] sprite, int sort, bool isTip)
        {
            ...
            if (isTip)
                tipMask = initMask(part, "TipMask", new Vector2(Mathf.Abs(TipAdjust), 0), new Vector2(BlastImg[0].bounds.size.x * ppu, TipMaskYScale), rend.sortingOrder + 3, rend.sortingOrder);
            else
        ...
        private GameObject initLaserMain(GameObject main, Vector3 initPosition, Sprite[] sprite, int sort)
        {
            ...
            mainMask = initMask(main, "MainMask", new Vector2(0, 0), new Vector2(1, MainMaskYScale), rend.sortingOrder, rend.sortingOrder - 1);
