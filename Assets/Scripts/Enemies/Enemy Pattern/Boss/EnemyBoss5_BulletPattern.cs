...
public class BulletPattern_EnemyBoss5_1A1 : BulletFactory, IBulletPattern
{
    ...
    {
        float rand = GameManager.RandomTest(0f, 360f);
...
public class BulletPattern_EnemyBoss5_1A2 : BulletFactory, IBulletPattern
{
    ...
    public IEnumerator ExecutePattern(UnityAction onCompleted)
    {
        ...
        while (_func_fireDelay.Invoke() > _targetDelay) {
            Vector3 pos = GetFirePos(0);
            var dir = GameManager.RandomTest(-6f, 6f);
            var interval = GameManager.RandomTest(0f, 24f);
...
public class BulletPattern_EnemyBoss5_1B1 : BulletFactory, IBulletPattern
{
    ...
    public IEnumerator ExecutePattern(UnityAction onCompleted)
    {
        while (true)
        {
            ...
            var dir = GameManager.RandomTest(-5f, 5f);
            if (SystemManager.Difficulty == GameDifficulty.Normal) {
...
public class BulletPattern_EnemyBoss5_1C1 : BulletFactory, IBulletPattern
{
    ...
    public IEnumerator ExecutePattern(UnityAction onCompleted)
    {
        while (true) {
            var pos = GetFirePos(0);
            if (SystemManager.Difficulty == GameDifficulty.Normal) {
                var dir = GameManager.RandomTest(-10f, 10f);
                ...
                var dir = GameManager.RandomTest(-8f, 8f);
...
public class BulletPattern_EnemyBoss5_1D1 : BulletFactory, IBulletPattern
{
    ...
    public IEnumerator ExecutePattern(UnityAction onCompleted)
    {
        ...
        yield return new WaitForMillisecondFrames(GameManager.RandomTest(0, 1200));
    ...
    public IEnumerator ExecutePattern(UnityAction onCompleted)
    {
        ...
        while (true)
        {
            for (var i = 0; i < 3; ++i)
            {
                ...
                var randDir = GameManager.RandomTest(-2f, 2f);
                if (SystemManager.Difficulty == GameDifficulty.Normal)
                {
...
public class BulletPattern_EnemyBoss5_2A2 : BulletFactory, IBulletPattern
{
    ...
    public IEnumerator ExecutePattern(UnityAction onCompleted)
    {
        var interval = GameManager.RandomTest(0f, 260f);
    ...
    public IEnumerator ExecutePattern(UnityAction onCompleted)
    {
        ...
        while (true) {
            for (int i = 1; i < 4; i++) {
                ...
                var dir = GameManager.RandomTest(-18f, 18f);
                if (SystemManager.Difficulty == GameDifficulty.Normal)
                {
...
{
    public BulletPattern_EnemyBoss5_2C2(EnemyObject enemyObject) : base(enemyObject) { }
    
    public IEnumerator ExecutePattern(UnityAction onCompleted)
    {
        int[] repeatNum = { 3, 3, 4 };
        int[] fireDelay = { 1800, 1000, 500 };
        int[] subFireDelay = { 500, 350, 200 };
        int[] subRepeatNum = { 3, 5, 7 };
        var rand = GameManager.RandomTest(0, 2) * 2 - 1;

        var currentNum = 0;
        while (currentNum < repeatNum[(int) SystemManager.Difficulty])
        {
            currentNum++;
            for (var i = 0; i < subRepeatNum[(int) SystemManager.Difficulty]; ++i)
            {
                for (var j = 0; j < 4; j++)
                {
                    var pos = GetFirePos(0);
                    var dir = (_enemyObject.m_CustomDirection[2] + 90f * j) * rand;

                    if (SystemManager.Difficulty == GameDifficulty.Normal)
                    {
                        CreateBullet(new BulletProperty(pos, BulletImage.PinkNeedle, 4.2f, BulletPivot.Fixed, dir, 3, 15f));
                    }
                    else if (SystemManager.Difficulty == GameDifficulty.Expert)
                    {
                        CreateBullet( new BulletProperty(pos, BulletImage.PinkNeedle, 4.2f, BulletPivot.Fixed, dir, 5, 9f));
                    }
                    else
                    {
                        CreateBullet(new BulletProperty(pos, BulletImage.PinkNeedle, 4.2f, BulletPivot.Fixed, dir, 9, 6f));
                        CreateBullet(new BulletProperty(pos, BulletImage.PinkLarge, 4.8f, BulletPivot.Fixed, dir + 45f));
                    }
                }
            ...
            rand *= -1;
            var delay = fireDelay[(int)SystemManager.Difficulty] - subFireDelay[(int)SystemManager.Difficulty];
