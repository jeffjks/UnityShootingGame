        ...
        while(true) {
            if (SystemManager.Difficulty == GameDifficulty.Normal) {
                ...
                for (int i = 0; i < 5; i++) {
                    ...
                    CreateBulletsSector(1, pos[0], 6.8f, CurrentAngle + 12f - i*8f, accel, number1[i], 12f);
                    CreateBulletsSector(1, pos[1], 6.8f, CurrentAngle - 12f + i*8f, accel, number1[i], 12f);
                ...
                for (int i = 0; i < 7; i++) {
                    pos[0] = BackgroundCamera.GetScreenPosition(m_FirePosition[0].position);
                    pos[1] = BackgroundCamera.GetScreenPosition(m_FirePosition[1].position);
                    CreateBulletsSector(1, pos[0], 6.8f, CurrentAngle + 16f - i*8f, accel, number2[i], 8f);
                    CreateBulletsSector(1, pos[1], 6.8f, CurrentAngle - 16f + i*8f, accel, number2[i], 8f);
                ...
                for (int i = 0; i < 7; i++) {
                    pos[0] = BackgroundCamera.GetScreenPosition(m_FirePosition[0].position);
                    pos[1] = BackgroundCamera.GetScreenPosition(m_FirePosition[1].position);
                    CreateBulletsSector(1, pos[0], 6.8f, CurrentAngle + 6f - i*6f, accel, number3[i], 6f);
                    CreateBulletsSector(1, pos[1], 6.8f, CurrentAngle - 6f + i*6f, accel, number3[i], 6f);
                    yield return new WaitForMillisecondFrames(140);
                }
                m_Shooting = false;
                yield return new WaitForMillisecondFrames(1800);
